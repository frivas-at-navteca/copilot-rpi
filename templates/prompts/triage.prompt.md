...
Reports are NEVER committed. Only code fixes enter version control.
...
- **Exhaustive discovery.** Use timestamp-based scan with `.last-triage` marker. Never assume how many reports exist. Present the full count before processing.
- **Never commit reports.** Reports stay on disk as local operational history. Only code fixes are committed. `docs/agents/`, `logs/`, and `scripts/agents/` are gitignored.
