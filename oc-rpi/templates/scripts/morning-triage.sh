...
# Designed for launchd/cron or manual invocation.
#
# ── Setup ──
#
...
if ! "$OPENCODE_BIN" run "echo ok" >/dev/null 2>&1; then
  echo "[$(date)] FATAL: OpenCode CLI auth failed in non-interactive mode."
  echo "  Fix: Authenticate opencode from an interactive terminal first."
  exit 1
fi
...
for PROJECT_DIR in "${PROJECTS[@]}"; do
  ...
  TRIAGE_COMMAND="$PROJECT_DIR/.opencode/commands/triage.md"
  if [ -f "$TRIAGE_COMMAND" ]; then
    PROMPT="Read and follow the instructions in $TRIAGE_COMMAND.
...
Write your final summary as your text output."
  ...
  while [ $RETRY_COUNT -le $MAX_RETRIES ]; do
    if cd "$PROJECT_DIR" && "$OPENCODE_BIN" run "$PROMPT" \
