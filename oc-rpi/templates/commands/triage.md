...
Process all overnight agent reports. Discovers every report exhaustively, checks for
agent failures, synthesizes findings, proposes an action plan, implements all fixes,
and commits reports for historical record.

## Input

If a report path is provided as `$ARGUMENTS`, process only the specified report path(s).
Otherwise, auto-discover all new/modified reports in `docs/agents/`.
If no reports found and no agent failures detected, report "all clear" and **STOP.**

## Step 1: Discovery

Find EVERY report and agent failure. No assumptions about which agents ran or how
many reports exist.
...
4. **Present discovery results:**
   ...
   Do NOT stop here — proceed directly to analysis unless there are ZERO reports and
   ZERO failures (in which case report "all clear" and **STOP**).
...
5. **Draft the action plan:**
   ...
   Group action items by report. Include ALL items.
   For each item: what to do, which files, expected outcome.
...
2. **Run verification sequentially** via bash.
...
- **Exhaustive discovery.** Use all three scan layers. Never assume how many reports exist.
- **Check for agent failures.** Scan `logs/` BEFORE analyzing reports.
- **Fix everything.** Categorize findings by severity, but implement 100% of action items.
  No deferring.
- **Read every report completely.** No skimming.
- **Two-commit strategy.** First commit: reports as-written. Second commit: code fixes.
  Never mix them.
- **shared-context.md integration.** Read before analysis, append triage entry after
  completion.
