---
description: Set up a new project using the oc-rpi blueprint
---
# Bootstrap New Project from oc-rpi Blueprint

You are setting up a new project using the oc-rpi blueprint repository.
The blueprint lives at `$ARGUMENTS`.

## Phase 1: Learn the Rules

Read these files from oc-rpi IN ORDER. Do not skip any.
...
1. `oc-rpi/templates/setup-checklist.md` — Your step-by-step guide.
2. `oc-rpi/templates/AGENTS.md.template` — Starting point for this project's AGENTS.md.
3. `oc-rpi/templates/opencode.json.template` — Starting point for `opencode.json`.
...
4. Create `opencode.json` — adapt from the template.
5. Create `.opencode/commands/` — copy command files and adjust.
6. Create `.opencode/agents/` — set up research and planning agents.
7. Add instruction globs to `opencode.json` `instructions` array.
...
filled in from everything you learned during setup:
...
[Decisions made during bootstrap — why certain template sections were kept,
removed, or adapted]

## Internalized Rules

[Key rules from patterns/quick-reference.md that are especially relevant
to this project]
