...
2. Use the explore subagent (`@explore`) to find relevant code, patterns, and docs.
...
11. Assess phase independence: mark phases that have no file overlap and no dependency on
    another phase's output as `[batch-eligible]`. These can be executed in parallel via
    separate `opencode run` processes.
